...
public class TodaysScheduleCursorAdapter extends CursorAdapter {
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        if(mSection==1 && rowSection.equals("1")) {
            ...
            desc.setText(
                    cursor.getString(
                            cursor.getColumnIndex(MedipalContract.MedicineEntry.MEDICINE_MEDICINE_NAME))
            );
            time.setText(
                    cursor.getString(
                            cursor.getColumnIndex(MedipalContract.MedicineEntry.MEDICINE_TIME)
                    )
            );
            subtitle.setText(
                    cursor.getString(
                            cursor.getColumnIndex(MedipalContract.MedicineEntry.MEDICINE_CONSUME_QUANTITY)
                    ) + cursor.getString(
                            cursor.getColumnIndex(MedipalContract.MedicineEntry.MEDICINE_DOSAGE)
                    )
            );
